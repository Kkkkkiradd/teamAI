...
class FileProcessor:
    ...
    def processor(self, keyField):
        ...
        names = list(map(lambda field: field.name, numDF.schema))
        x = list(map(lambda row: list(row), numDF.collect()))
        y = list(zip(*map(lambda row: list(row), notNullDF.select(keyField).collect())))[0]
        # print(len(x), len(y))
        return x, y, names
